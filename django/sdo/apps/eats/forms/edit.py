...
class GenericFormSet (object):
    ...
    def create_forms (self, assertions, extra_data, post_data, new_forms, inline):
    ...
    def _create_instance_form (self, assertion, extra_data, post_data):
        ...
        kw_args = {'initial': initial_data, 'prefix': prefix, 'data': post_data}
    ...
    def _create_inline_instance_form (self, inline_name, instance, form_class, post_data):
    ...
    def _create_inline_new_form (self, property_id, inline_name, form_class, post_data, count, initial=None):
        """Return a form for adding a new inline object, using the extra data."""
        prefix = '%s_%s_%s_new_%s' % (self.assertion_type, str(property_id), inline_name, str(count))
...
class NameRelationshipFormSet (GenericFormSet):
    ...
    def create_forms (self, assertions, extra_data, post_data, new_forms, inlines):
        """Create the instance and new forms."""
        for assertion in assertions:
            key = assertion.name_relationship.name.id
            # Remove the current name from the list of possible
            # related names.
            extra_data['related_names'] = extra_data['names'].exclude(assertion=assertion)
...
class PropertyForm (EditForm):
    ...
    authority_record = forms.ModelChoiceField(AuthorityRecord.objects.all(), required=True,
                                              error_messages={'required': 'An authority record must be selected'},
    ...
    def __init__ (self, authority_records, extra_data, *args, **kw_args):
        ...
        self.fields['authority_record'].choices = create_choice_list(authority_records, True)
...
class AuthorityRecordCreateForm (forms.ModelForm):
    ...
    def __init__ (self, authorities, *args, **kw_args):
        ...
        self.fields['authority'].choices = create_choice_list(authorities, True)
    ...
    def clean (self):
        ...
        if not self.errors and self.cleaned_data:
            ...
            if not (record_id or record_url):
                raise forms.ValidationError('You must fill in at least one of the record ID and record URL fields')
        return self.cleaned_data
...
class AuthorityRecordSearchForm (forms.Form):
    ...
    search_record_id = forms.CharField(max_length=200, required=False, label='Record ID')
    search_record_url = forms.CharField(max_length=400, required=False, label='Record URL')

    def __init__ (self, authorities, *args, **kw_args):
        super(AuthorityRecordSearchForm, self).__init__(*args, **kw_args)
        self.fields['search_authority'].queryset = authorities
        self.fields['search_authority'].choices = create_choice_list(authorities, True)
...
class EntityTypeForm (InlinePropertyForm):
    ...
    def __init__ (self, authority_records, extra_data, *args, **kw_args):
        super(EntityTypeForm, self).__init__(authority_records, extra_data, *args, **kw_args)
    ...
    def clean (self):
        ...
        if not self.errors and self.cleaned_data:
            ...
            if entity_type_authority != authority_record.authority:
                raise forms.ValidationError('The entity type must be associated with the same authority as the authority record.')
        return self.cleaned_data
...
class ExistenceForm (forms.ModelForm):
    ...
    delete = forms.BooleanField(required=False)
    authority_record = forms.ModelChoiceField(AuthorityRecord.objects.all(), widget=forms.HiddenInput)
    authority_record_name = forms.CharField(widget=forms.TextInput(attrs={'disabled': 'disabled'}), required=False)
    is_preferred = forms.BooleanField(required=False)
    ...
    def __init__ (self, authority_records, extra_data, *args, **kw_args):
        ...
        if kw_args.get('instance'):
            try:
                authority_record_name = unicode(self.instance.assertion.get().authority_record)
            except:
    ...
    def clean (self):
        ...
        if self.cleaned_data.get('delete'):
            ...
            if has_dependent_records:
                raise forms.ValidationError('An existence may not be deleted if its authority record is associated with other properties.')
        return self.cleaned_data
...
class NameForm (PropertyForm):
    ...
    def __init__ (self, authority_records, extra_data, *args, **kw_args):
        ...
        self.fields['name_type'].choices = create_choice_list(name_type_choices, True)
...
class NamePartForm (EditForm):
    ...
    def __init__ (self, extra_data, *args, **kw_args):
        ...
        name_part_type_choices = extra_data.get('name_part_type_choices', name_part_types)
        self.fields['name_part_type'].choices = create_choice_list(name_part_type_choices)
...
class NameRelationshipForm (InlinePropertyForm):
    ...
    def __init__ (self, authority_records, extra_data, *args, **kw_args):
        ...
        self.fields['name_relationship_type'].choices = create_choice_list(name_relationship_types)
    ...
    def clean (self):
        ...
        if not self.errors and self.cleaned_data:
            ...
            if relationship_type_authority != name_authority:
                error_messages.append('The name relationship type must be associated with the same authority as the authority record.')
            if name_authority != related_name_authority:
                error_messages.append('The related name must be associated with the same authority as the name.')
            if error_messages:
...
class EntityRelationshipForm (InlinePropertyForm):
    ...
    related_entity = forms.ModelChoiceField(Entity.objects.all(), widget=forms.HiddenInput)
    related_entity_name = forms.CharField(widget=forms.TextInput(attrs={'disabled': 'disabled'}), required=False)
    unchecked_fields = ['authority_record', 'related_entity_name']

    def __init__ (self, authority_records, extra_data, *args, **kw_args):
        super(EntityRelationshipForm, self).__init__(authority_records, extra_data, *args, **kw_args)
        ...
        self.fields['entity_relationship_type'].choices = create_choice_list(entity_relationship_types)
        self.fields['entity_relationship_type'].error_messages={'required': 'A relationship type must be selected'}
        ...
        if kw_args.get('instance'):
            try:
                related_entity_name = self.instance.related_entity.get_single_name(extra_data.get('user_prefs'))
            except:
    ...
    def clean (self):
        ...
        if not self.errors and self.cleaned_data:
            entity_relationship_type_authority = self.cleaned_data['entity_relationship_type'].authority
            authority_record = self.cleaned_data['authority_record']
            if entity_relationship_type_authority != authority_record.authority:
                raise forms.ValidationError('The entity relationship must be associated with the same authority as the authority record.')
        return self.cleaned_data
...
class EntityRelationshipNoteForm (EditForm):
    ...
    delete = forms.BooleanField(required=False)
    entity_relationship = forms.ModelChoiceField(EntityRelationship.objects.all(), widget=forms.HiddenInput)
    note = forms.CharField(widget=NoteWidget, required=False)
...
class EntityNoteForm (InlinePropertyForm):
    ...
    def __init__ (self, authority_records, extra_data, *args, **kw_args):
        super(EntityNoteForm, self).__init__(authority_records, extra_data, *args, **kw_args)
        self.fields['note'].error_messages['required'] = 'The note may not be empty'
