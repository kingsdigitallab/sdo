...
to_eac_xslt = etree.parse(os.path.join(app_path, 'xsl/eatsml-to-eac-individual.xsl'))
...
def display_entity_eac (request, entity_id, authority_record_id):
    ...
    try:
        ...
        raise Http404
    try:
        authority_record = AuthorityRecord.objects.get(pk=int(authority_record_id))
...
def get_record_search_results (authority_id, record_id, record_url):
    ...
    queries = []
    if record_id:
        queries.append(Q(assertions__authority_record__authority_system_id=record_id))
    if record_url:
        queries.append(Q(assertions__authority_record__authority_system_url=record_url))
    Qs = None
    for query in queries:
        if Qs:
            Qs = Qs | query
            ...
            Qs = query
    results = set(Entity.objects.filter(Qs, assertions__authority_record__authority__pk=authority_id).distinct())
    return results
