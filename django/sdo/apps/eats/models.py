...
class Authority (models.Model):
    ...
    default_date_period = models.ForeignKey('DatePeriod')
    default_date_type = models.ForeignKey('DateType')
    default_language = models.ForeignKey('Language')
    default_script = models.ForeignKey('Script')
...
class EntityType (models.Model):
    ...
    entity_type = models.ForeignKey(EntityTypeList)
...
class EntityRelationshipTypeRelationship (models.Model):
    child_entity_relationship_type = models.ForeignKey(
        EntityRelationshipType, related_name='entityrelationshiptype_child_set')
...
class EntityRelationship (models.Model):
    ...
    entity_relationship_type = models.ForeignKey(EntityRelationshipType)
...
class NamePartType (models.Model):
    ...
    authority = models.ForeignKey(Authority)
    system_name_part_type = models.ForeignKey(SystemNamePartType)
    last_modified = models.DateTimeField(auto_now=True)
...
class NamePart (models.Model):
    name = models.ForeignKey(Name, related_name='name_parts')
    name_part_type = models.ForeignKey(NamePartType)
...
class NameRelationship (models.Model):
    name = models.ForeignKey(Name, related_name='name_start_relationships')
    related_name = models.ForeignKey(Name,
                                     related_name='name_end_relationships')
    name_relationship_type = models.ForeignKey(NameRelationshipType)
...
class UserDefinedPropertyConstrainedValue (models.Model):
    property = models.ForeignKey(UserDefinedProperty)
...
class GenericProperty (models.Model):
    ...
    constrained_value = models.ForeignKey(UserDefinedPropertyConstrainedValue)
...
class PropertyAssertion (models.Model):
    entity = models.ForeignKey(Entity, related_name='assertions')
    ...
    existence = models.ForeignKey(Existence, null=True, blank=True,
                                  unique=True, related_name='assertion')
...
class Date (models.Model):
    assertion = models.ForeignKey(PropertyAssertion, related_name='dates')
    ...
    end_date_type = models.ForeignKey(DateType, verbose_name='Type', null=True,
                                      related_name='end_date_type_set')
...
class SearchName (models.Model):
    ...
    name."""
    entity = models.ForeignKey(Entity, related_name='search_names')
    name = models.ForeignKey(Name, related_name='search_names')
    name_form = models.CharField(max_length=800)


class UserProfile (models.Model):
    user = models.ForeignKey(User) # Required by Django
...
class RegisteredImport (models.Model):
    importer = models.ForeignKey(User)
