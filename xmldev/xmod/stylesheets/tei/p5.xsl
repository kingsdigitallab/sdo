<?xml version="1.0"?>
<!--  $Id$ -->
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:tei="http://www.tei-c.org/ns/1.0"
  xmlns:xmm="http://www.cch.kcl.ac.uk/xmod/menu/1.0" xmlns:xmmf="http://www.cch.kcl.ac.uk/xmod/metadata/files/1.0"
  xmlns:xmp="http://www.cch.kcl.ac.uk/xmod/properties/1.0" xmlns:xmt="http://www.cch.kcl.ac.uk/xmod/tei/1.0"
  ...
  <xsl:template match="//tei:TEI/tei:teiHeader/tei:fileDesc/tei:titleStmt/*[self::tei:author|self::tei:editor|self::tei:respStmt]" mode="pagehead">
    <xsl:apply-templates mode="pagehead" />
  ...
  <xsl:template match="//tei:TEI/tei:teiHeader/tei:fileDesc/tei:titleStmt/tei:respStmt/tei:resp" mode="pagehead">
    <xsl:apply-templates />
  ...
  <xsl:template match="tei:p">
    <xsl:choose>
      <xsl:when test="tei:graphic[@xmt:type='thumb-caption']">
        <xsl:for-each select="tei:graphic[@xmt:type='thumb-caption'][string(@url)]">
    ...
    <xsl:choose>
      ...
      <xsl:when test="../@type='figure-list'">
        ...
        <xsl:choose>
          <xsl:when test="tei:p">
            <dd>
              <xsl:apply-templates select="text()|*[not(local-name()='tei:graphic')]" />
            </dd>
          </xsl:when>
          <xsl:otherwise>
            <dd>
...
-->
  ...
  <xsl:template name="consistency-test">
    ...
    <xsl:for-each select="tei:row[position()>1]">
      <xsl:variable name="cur-cell-count" select="count(tei:cell) + sum(tei:cell/@cols) - count(tei:cell/@cols)" />
  ...
  <xsl:template name="table-complexDisplay">
    ...
    <xsl:variable name="number-of-cells"
      select="count(tei:row[position() = 1]/tei:cell) + sum(tei:row[position() = 1]/tei:cell/@cols) - count(tei:row[position() = 1]/tei:cell/@cols)" />
  ...
  <xsl:template name="cell-att">
    ...
    <xsl:variable name="con-cell-position"
      select="count(preceding-sibling::tei:cell)  + sum(preceding-sibling::tei:cell/@cols) - count(preceding-sibling::tei:cell/@cols) + 1" />
    ...
    <xsl:choose>
      ...
      <xsl:when test="$count > 0">
        ...
        <xsl:variable name="new-count">
          <xsl:choose>
            <xsl:when test="ancestor::tei:table/tei:row[position() = $row]/tei:cell[position() = $cell]">
        ...
        <xsl:variable name="new-pos">
          <xsl:choose>
            <xsl:when test="ancestor::tei:table/tei:row[position() = $row and $context-row > position()]/tei:cell[position() = $cell and @rows]">
              <!-- Cases where preceding-siblings of the cell being tested have colspan -->
              <xsl:variable name="pre-cols"
                select="sum(ancestor::tei:table/tei:row[position() = $row]/tei:cell[position() = $cell]/preceding-sibling::tei:cell[$cell > position()]/@cols)" />
              ...
              <xsl:choose>
                <xsl:when
                  test="$pos >= $cell + $pre-cols and $row + ancestor::tei:table/tei:row[position() = $row]/tei:cell[position() = $cell]/@rows - 1 >= $context-row">
                  <!-- 
                  ...
                  <xsl:variable name="step">
                    <xsl:choose>
                      <xsl:when test="ancestor::tei:table/tei:row[position() = $row]/tei:cell[position() = $cell and @cols]">
                        <xsl:value-of select="ancestor::tei:table/tei:row[position() = $row]/tei:cell[position() = $cell]/@cols" />
                      </xsl:when>
  ...
  <xsl:template name="showFigure">
    ...
    <!-- folder info -->
    <xsl:variable name="img-folder" select="$xmp:images-path" />
    ...
    <xsl:choose>
      ...
      <xsl:when test="ancestor::xmt:ulist[@type='figure-full']">
      ...
      <xsl:when test="ancestor::xmt:ulist[@type='figure-thumb']">
      ...
      <xsl:when test="ancestor::xmt:ulist[@type='figure-list']">
        <img>
