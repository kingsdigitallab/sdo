<xsl:stylesheet version="2.0" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl"
    ...
    <xsl:variable name="uniqueID" select="concat($record, '_', $file)"/>

    <xsl:template match="/aggregation/response/result">
        <xsl:apply-templates select="doc"/>
    </xsl:template>

    <xsl:template match="doc">
        <xsl:choose>
            <xsl:when test="child::str[@name = 'uniqueId'] = $uniqueID">
                <nextprev>

                    <xsl:variable name="prevFile"
                        select="preceding-sibling::doc[1]/child::str[@name = 'fileId']"/>
                    <xsl:variable name="prevRecord"
                        select="preceding-sibling::doc[1]/child::str[@name = 'recordId']"/>
                    ...
                    <prevLink>
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:value-of
                                    select="concat('../', $prevFile, '/', $prevRecord)"
                                />
                            </xsl:otherwise>
                        </xsl:choose>
                    </prevLink>
                    ...
                    <xsl:variable name="nextRecord"
                        select="following-sibling::doc[1]/child::str[@name = 'recordId']"/>
                    ...
                    <nextLink>
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:value-of
                                    select="concat('../', $nextFile, '/', $nextRecord)"
                                />
                            </xsl:otherwise>
                        </xsl:choose>
                    </nextLink>
