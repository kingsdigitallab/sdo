<xsl:stylesheet version="2.0" xmlns:xmg="http://www.cch.kcl.ac.uk/xmod/global/1.0"
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>
            ...
            <xd:p>
                <xd:b>Input:</xd:b>
                <xd:b>Solr fields</xd:b> dateShort has form "1918-01-17" </xd:p>
        </xd:desc>
    </xd:doc>
    ...
    <xsl:variable name="heading_string">
        <xsl:choose>
            <xsl:when test="$pattern = 'cd'">
                <xsl:text>Correspondence and Diaries</xsl:text>
            </xsl:when>
            <xsl:when test="$pattern = 'correspondence'">
                <xsl:text>Correspondence</xsl:text>
            </xsl:when>
            <xsl:when test="$pattern = 'diaries'">
                <xsl:text>Diaries</xsl:text>
            </xsl:when>
            <xsl:when test="$pattern = 'lb'">
                <xsl:text>Lessonbooks</xsl:text>
            </xsl:when>
            <!-- But Nb. no lb browse by date yet -->
            <xsl:when test="$pattern = 'other'">
                <xsl:text>Other Materials</xsl:text>
            </xsl:when>
        </xsl:choose>
    </xsl:variable>
                                    ...
                                    <ul id="date">
                                        ...
                                        <xsl:for-each-group select="current-group()"
                                            ...
                                            <xsl:variable name="url">
                                                <xsl:choose><!-- because no "summary" step when browsing diaries -->
                                                  <xsl:when test="$pattern ='diaries'">
                                                  <xsl:value-of
