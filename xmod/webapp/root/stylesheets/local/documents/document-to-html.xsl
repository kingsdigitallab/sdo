...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns="http://www.w3.org/1999/xhtml"
    ...
    <li>
      ...
      <xsl:choose>
        <xsl:when test="$previous">
          <a href="{concat($xmg:pathroot, xms:path-separator($previous/str[@name='url']), $previous/str[@name='url'])}"
            >Previous</a>
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template match="tei:rs[@key]">
    <xsl:variable name="display-name" select="/*/eats/entities/entity[keys/key = current()/@key]/names/name[1]" />
