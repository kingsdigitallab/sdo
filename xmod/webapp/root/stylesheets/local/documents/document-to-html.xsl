...
<xsl:stylesheet version="2.0" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:sdo="http://www.cch.kcl.ac.uk/schenker"
  ...
  <xsl:template name="xms:pagehead">
    <div class="pageHeader">
      ...
      <div class="t01">
        <h1>
          <xsl:if test="/aggregation/sdo:recordCollection/sdo:collectionDesc/sdo:source/child::*[1]">
    ...
    <li>
      ...
      <xsl:choose>
        <xsl:when test="$previous">
          <a href="{concat($xmg:pathroot, xms:path-separator($previous/str[@name='url']), $previous/str[@name='url'])}">Previous</a>
        </xsl:when>
        <xsl:otherwise>
      ...
      <xsl:choose>
        <xsl:when test="$next">
          <a href="{concat($xmg:pathroot, xms:path-separator($next/str[@name='url']), $next/str[@name='url'])}">Next</a>
        </xsl:when>
        <xsl:otherwise>
