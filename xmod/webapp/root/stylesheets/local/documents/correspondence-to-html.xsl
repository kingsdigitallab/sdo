...
<xsl:stylesheet exclude-result-prefixes="*"
  ...
  <xsl:template name="browsing-nav">
    ...
    <ul>
      <xsl:apply-templates select="$current-item" mode="browse-by-date"/>
      <xsl:apply-templates select="$current-item" mode="browse-by-item-type"/>
      <xsl:apply-templates select="$current-item" mode="browse-by-correspondence"/>
      ...
      <ul>
        <xsl:for-each select="arr[@name='correspondence']/str">
          ...
          <xsl:call-template name="browse-for-type">
            ...
            <xsl:with-param name="previous" select="../../preceding-sibling::doc[arr[@name='correspondence']/str[substring-before(., ' ')=$key]][1]"/>
            <xsl:with-param name="next" select="../../following-sibling::doc[arr[@name='correspondence']/str[substring-before(., ' ')=$key]][1]"/>
          </xsl:call-template>
        </xsl:for-each>
      </ul>
    </li>
      ...
      <ul>
        <xsl:for-each select="arr[@name='correspondent']/str">
          ...
          <xsl:call-template name="browse-for-type">
            ...
            <xsl:with-param name="previous" select="../../preceding-sibling::doc[arr[@name='correspondent']/str[substring-before(., ' ')=$key]][1]"/>
            <xsl:with-param name="next" select="../../following-sibling::doc[arr[@name='correspondent']/str[substring-before(., ' ')=$key]][1]"/>
          </xsl:call-template>
        </xsl:for-each>
      </ul>
    </li>
    ...
    <li>
      ...
      <ul>
        <xsl:for-each select="arr[@name='recipient']/str">
          ...
          <xsl:call-template name="browse-for-type">
            <xsl:with-param name="type" select="substring-after(., ' ')"/>
            <xsl:with-param name="previous" select="../../preceding-sibling::doc[arr[@name='recipient']/str[substring-before(., ' ')=$key]][1]"/>
            <xsl:with-param name="next" select="../../following-sibling::doc[arr[@name='recipient']/str[substring-before(., ' ')=$key]][1]"/>
          </xsl:call-template>
        </xsl:for-each>
      </ul>
    </li>
