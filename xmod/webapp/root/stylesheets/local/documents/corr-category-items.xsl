<xsl:stylesheet version="2.0" xmlns:xmg="http://www.cch.kcl.ac.uk/xmod/global/1.0"
    ...
    <xsl:param name="type"/><!-- passed in single-string form, eg. Bank_document, so we expand it below -->
    
    <xsl:variable name="xmg:title">
        <xsl:text>Browse Correspondence By Category</xsl:text>
    </xsl:variable>
    <xsl:variable name="root" select="/"/>
    <xsl:variable name="expanded_type" select="replace($type, '_', ' ')"/>
    
    <xsl:template name="xms:content">
        <h2><xsl:value-of select="$expanded_type"></xsl:value-of></h2>
        <ul>
